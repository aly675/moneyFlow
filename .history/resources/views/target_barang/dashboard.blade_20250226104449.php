            ...
            @foreach($targetBarangs as $target)
                {{-- <a href="{{ route('target_barang.show', $target->id) }}"> --}}
                    <div class="target-card phone">
                        ...
                        <div class="price-info">Rp. {{ number_format($target->saldo_t1erkumpul, 0, ',', '.') }}/Rp. {{ number_format($target->harga_barang, 0, ',', '.') }}</div>
                        <div class="persen">{{ round(($target->saldo_terkumpul / $target->harga_barang) * 100) }}%</div>
                    </div>
                {{-- </a> --}}
            @endforeach
        </div>
