<?php
use Illuminate\Support\Facades\Route;
use App\Http\Controllers\AuthController;
use App\Http\Controllers\IndexController;
use App\Http\Middleware\UserIsAuthenticated;
use App\Http\Middleware\RedirectIfAuthenticated;
use App\Http\Controllers\PemasukanPengeluaranController;
use App\Http\Controllers\TargetBarangController;

// Rute untuk dashboard
Route::middleware([UserIsAuthenticated::class])->group(function () {
    // Route::get('/', function () {
    //     return view('dashboard');
    // })->name('dashboard');

    Route::get('/', [IndexController::class, 'index'])->name('dashboard');

    Route::get('/dashboard/data', [IndexController::class, 'getData'])->name('dashboard.data');

    // Route untuk menyimpan pemasukan
    Route::get('/pemasukan', function(){
        return view('pemasukan');
    })->name('pemasukan_page');
    Route::post('/simpan-pemasukan', [PemasukanPengeluaranController::class, 'simpanPemasukan'])->name('simpan.pemasukan');

    Route::get('/pengeluaran', function(){
        return view('pengeluaran');
    })->name('pengeluaran_page');
    Route::post('/simpan-pengeluaran', [PemasukanPengeluaranController::class, 'simpanPengeluaran'])->name('simpan.pengeluaran');

    Route::get('/api/target-barang', [PemasukanPengeluaranController::class, 'getTargetBarang'])->name('target.barang');


    Route::prefix('menejemen_anggaran/')->group(function(){
        Route::get('', [IndexController::class, 'menejemen_anggaran_dashboard'])->name('manejemen.anggaran');

        Route::prefix('kategori/')->group(function(){
            route::get('UangTunai',[IndexController::class, 'uangTunai_page'] )->name('kategori.UangTunai');
            route::get('Mandiri',[IndexController::class, 'mandiri_page'] )->name('kategori.Mandiri');
            route::get('Gopay',[IndexController::class, 'gopay_page'] )->name('kategori.Gopay');
            route::get('Dana',[IndexController::class, 'dana_page'] )->name('kategori.Dana');
            route::get('BRI',[IndexController::class, 'BRI_page'] )->name('kategori.BRI');
            route::get('BCA',[IndexController::class, 'BCA_page'] )->name('kategori.BCA');

            // Rute untuk menampilkan halaman edit
            Route::get('{type}/{id}/edit', [PemasukanPengeluaranController::class, 'EditByKategori'])->name('edit.pemasukan.pengeluaran.page');
            // Rute untuk memperbarui data
            // Route::put('{type}/{id}/edit', [PemasukanPengeluaranController::class, 'updateData'])->name('update.pemasukan.pengeluaran');
            Route::put('{type}/{id}/edit', [PemasukanPengeluaranController::class, 'updateData'])->name('update.pemasukan.pengeluaran');
         });

        });
    Route::prefix('target-barang')->group(function () {
        Route::get('/', [TargetBarangController::class, 'index'])->name('target_barang.index');            Route::get('/create', [TargetBarangController::class, 'create'])->name('target_barang.create');
            Route::post('/', [TargetBarangController::class, 'store'])->name('target_barang.store');
            Route::get('/{id}', [TargetBarangController::class, 'show'])->name('target_barang.show');
            Route::post('/{id}/pemasukan', [TargetBarangController::class, 'addPemasukan'])->name('target_barang.add_pemasukan');
        });


    route::prefix('rekap/')->group(function (){
        route::get('realTime',[IndexController::class, 'realTime_page'])->name('rekap.realTime');
        route::get('bulanan',[IndexController::class, 'bulanan_page'])->name('rekap.bulanan');
        Route::get('data/realtime', [IndexController::class, 'getRealTimeData'])->name('realtime.data');
        Route::get('data/bulanan', [IndexController::class, 'getBulananData'])->name('bulanan.data');
    });
});

// Terapkan middleware RedirectIfAuthenticated pada rute login dan registrasi
Route::middleware([RedirectIfAuthenticated::class])->group(function () {
    // Route::get('/login', [AuthController::class, 'showLoginForm'])->name('login_page');
    // Route::post('/login', [AuthController::class, 'login'])->name('login_submit');

    // // Register Routes
    // Route::get('/register', [AuthController::class, 'showRegistrationForm'])->name('register_page');
    // Route::post('/register', [AuthController::class, 'register'])->name('register_submit');

  // web.php
    Route::get('/loginRegis', [AuthController::class, 'showLoingRegisForm'])->name('loginRegis');
    Route::post('/loginRegis/login', [AuthController::class, 'login'])->name('login.submit');
    Route::post('/loginRegis/register', [AuthController::class, 'register'])->name('register.submit');
});

// Route::get('data/realtime', [IndexController::class, 'getRealTimeData'])->name('realtime.data');

// Logout Route
Route::post('/logout', [AuthController::class, 'logout'])->name('logout');
