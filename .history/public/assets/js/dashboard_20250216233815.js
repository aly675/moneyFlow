    ...
    function fetchData(tanggal) {
        ...
        $.ajax({
            ...
            success: function(response) {
                ...
                $('#totalPemasukan').text(response.totalPemasukan.toLocaleString());
                $('#totalPengeluaran').text(response.totalPengeluaran.toLocaleString());
                $('#balance').text(response.balance.toLocaleString());

                // Kosongkan container data
                $('#dataContainer').empty();

                // Tambahkan data pemasukan
                response.pemasukan.forEach(function(item) {
                    $('#dataContainer').append(`
                        <div class="show-data">
                            ...
                            <div class="show-money-pemasukan">
                                <p class="bi bi-plus">${item.jumlah.toLocaleString()}</p>
                            </div>
                        </div>
                    `);
                });
                ...
                response.pengeluaran.forEach(function(item) {
                    $('#dataContainer').append(`
                        <div class="show-data">
                            ...
                            <div class="show-money-pengeluaran">
                                <p class="bi bi-dash">${item.jumlah.toLocaleString()}</p>
                            </div>
                        </div>
                    `);
                });
