...
// Fungsi untuk memperbarui teks yang diformat untuk target barang
function updateFormattedTextTargetBarang() {
    const input = document.getElementById("nominalTargetBarang");
    const formattedText = document.getElementById("formattedTextTargetBarang");
    let value = parseFloat(input.value) || 0; // Menggunakan 0 jika input tidak valid

    // Ambil nilai dari input jumlah pemasukan
    const maxPemasukan = parseFloat(document.getElementById("jumlah").value) || 0;

    // Pastikan nilai tidak lebih dari jumlah pemasukan
    if (value > maxPemasukan) {
        input.value = maxPemasukan; // Batasi inputan
        value = maxPemasukan; // Update value untuk format
    }
