...
function validateInput(input) {
    ...
    input.value = formatRupiah(inputValue);
}

// Fungsi untuk mendapatkan nilai yang valid untuk dikirim ke server
function getValidNumber() {
    const input = document.getElementById('jumlah');
    const value = input.value.replace(/[^0-9]/g, ''); // Menghapus format mata uang
    return value ? parseInt(value) : 0; // Mengembalikan angka atau 0 jika kosong
}

// Contoh pengiriman data
function simpanPemasukan() {
    const jumlah = getValidNumber(); // Mendapatkan nilai yang valid
    if (jumlah > 0) { // Pastikan jumlah lebih dari 0
        // Lakukan pengiriman data ke server menggunakan AJAX atau form submission
        console.log(jumlah); // Untuk debugging, lihat nilai yang akan dikirim

        // Contoh AJAX request (gunakan sesuai kebutuhan Anda)
        fetch('/simpan-pemasukan', {
            method: 'POST',
            headers: {
