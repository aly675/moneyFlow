...
document.addEventListener("DOMContentLoaded", function() {
    ...
    window.toggleDropdown = function() {        const dropdownContent = document.getElementById('paymentOptions');
    ...
    document.querySelector('.save-btn').addEventListener('click', function() {
        ...
        .then(result => {
            if (result.success) {
                ...
                document.getElementById('date').value = new Date().toISOString().split('T')[0]; // Reset tanggal
                document.querySelector('input[type="text"]').value = ''; // Reset judul
                document.querySelector('input[type="number"]').value = ''; // Reset jumlah
                document.getElementById('notes').value = ''; // Reset deskripsi
                // Reset kategori dropdown
                document.querySelector('.dropdown-header').textContent = 'Pilih Kategori';
                paymentOptions.forEach(opt => opt.classList.remove('selected'));
            } else {
